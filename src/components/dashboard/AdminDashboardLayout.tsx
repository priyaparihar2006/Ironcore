...
export const AdminDashboardLayout: React.FC<AdminDashboardLayoutProps> = ({ children }) => {
  ...
  const navItems = [
    ...
    { label: 'User Directory & Roles', path: '/admin/users', icon: Users },
    { label: 'Trainer Management', path: '/admin/trainers', icon: Shield },
    { label: 'Membership Tiers', path: '/admin/memberships', icon: CreditCard },
    { label: 'Revenue & Payments', path: '/admin/payments', icon: DollarSign },
  ];
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-neutral-200/80 p-6 flex-shrink-0 min-h-screen sticky top-0 z-30">
        ...
        <div className="flex items-center justify-between mb-8 px-2">
          <Link to="/" className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-red-950 rounded-lg flex items-center justify-center shadow-md">
              <div className="w-3.5 h-3.5 border-2 border-red-400 rounded-full"></div>
            </div>
            <span className="text-xl font-black tracking-tighter text-[#080512]">
              IronCore
            </span>
          </Link>
          <span className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-red-100 text-red-800">
            Admin
          </span>
        </div>
        ...
        {/* Switch to Athlete or Trainer View */}
        <div className="my-4 p-3 bg-neutral-50 rounded-2xl border border-neutral-200/80 space-y-2">
          <Link
            to="/dashboard"
          ...
          <Link
            to="/trainer"
            className="flex items-center gap-2 text-xs font-bold text-purple-700 hover:underline pt-2 border-t border-neutral-200"
          >
        ...
        <div className="pt-4 border-t border-neutral-200/70">
          <div className="flex items-center gap-3 p-2 rounded-2xl bg-neutral-50 border border-neutral-200/60 mb-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[10px] text-red-700 font-bold uppercase">System Administrator</div>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-neutral-200/80 sticky top-0 z-20 px-4 sm:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-red-700 px-2 py-0.5 rounded-full bg-red-50">
