...
export const SignupPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-brand-bg)] flex flex-col justify-center py-8 sm:py-14 px-4 md:px-8 lg:px-12">
      <div className="max-w-5xl w-full mx-auto bg-white rounded-lg sm:rounded-xl border border-[var(--color-border-main)] shadow-sm/80 shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 p-8 sm:p-12 lg:p-14 flex flex-col justify-center">
          <div className="max-w-md w-full mx-auto">
            ...
            {error && (
              <div className="mb-6 p-6 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm flex items-start gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <ValidationInput {...validation.field('name', 'Full name')}
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-xl border border-[var(--color-border-main)] shadow-sm bg-neutral-50/50 text-[var(--color-text-main)] placeholder-neutral-400 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <ValidationInput {...validation.field('dateOfBirth', 'Date of birth')}
                      ...
                      className="w-full pl-10 pr-4 py-3 rounded-xl border border-[var(--color-border-main)] shadow-sm bg-neutral-50/50 text-[var(--color-text-main)] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                ...
                <div>
                  ...
                  <ValidationSelect {...validation.field('gender', 'gender')}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-[var(--color-border-main)] shadow-sm bg-neutral-50/50 text-[var(--color-text-main)] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                  >
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border text-center ${
                        fitnessGoal === goal
                          ...
                          : 'bg-[var(--color-brand-bg)] text-neutral-700 border-[var(--color-border-main)] hover:bg-neutral-100'
                      }`}
                    >
                      {goal}
                    </button>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <ValidationInput {...validation.field('password', 'Password')}
                      ...
                      className="w-full pl-10 pr-10 py-3 rounded-xl border border-[var(--color-border-main)] shadow-sm bg-neutral-50/50 text-[var(--color-text-main)] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                    maxLength={256} autoComplete="new-password" />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      className="absolute inset-y-0 right-0 pr-3 flex items-center text-[var(--color-text-muted)] hover:text-neutral-700"
                    >
              ...
              <div className="pt-2">
                <label className="flex items-start gap-2.5 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 mt-0.5 rounded border-neutral-300 text-[var(--color-text-main)] focus:ring-blue-500"
                  />
                  <span className="text-xs text-neutral-600 leading-tight">
                    I agree to the <span className="font-bold text-[var(--color-text-main)]">IronCore Club Terms of Service</span> and acknowledge the gym liability policy.
                  </span>
                </label>
              </div>

              {validation.error('agreeTerms') && <p role="status" className="text-xs text-red-700">{validation.error('agreeTerms')}</p>}
              <p className="text-xs text-[var(--color-text-muted)]">Use 8?128 characters with uppercase, lowercase, a number and a special character. International phone numbers need a +country code; unprefixed numbers use Indian mobile rules.</p>
              {/* Submit Button */}
              <button
                type="submit"
                disabled={loading}
                id="signup-submit-btn"
                className="w-full py-4 rounded-lg bg-[var(--color-primary)] text-[var(--color-text-main)] font-bold text-sm sm:text-base flex items-center justify-center gap-2 shadow-sm hover:bg-neutral-800 active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer mt-2"
              >
                {loading ? (
                  <>
            ...
            <div className="mt-6 text-center text-sm text-neutral-600">
              ...
              <Link to="/login" className="font-bold text-[var(--color-text-main)] hover:underline underline-offset-4">
                Sign In
              </Link>
            </div>
